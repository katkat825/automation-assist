...
def run_all_checks(
    ...
    # and only if an Accessible Path scene was actually detected.
    if dual_path:
        if jaws_checks.has_accessible_path(data):
            sections.extend(jaws_checks.run_jaws_checks(data))
        else:
            jaws_skip = Section("JAWS / Screen Reader Checks")
            jaws_skip.add(
                "warn",
                "Dual-path was checked but no scene titled 'Accessible Path' was found — "
                "JAWS checks skipped. Rename the accessible scene or uncheck Dual-path.",
            )
            sections.append(jaws_skip)
